...
function compareObjects(obj1, obj2) {
  ...
  return keysArr.every((key) => obj2[key] === obj1[key]);
}
...
function isEmptyObject(obj) {
  ...
  // const keysArr = Object.keys(obj);
  // return keysArr.every((key) => !obj.hasOwnProperty(key));
}
...
function makeWord(lettersObject) {
  const newArr = Object.entries(lettersObject);
  const wordArr = [];
  newArr.forEach(([letter, positions]) => {
    positions.forEach((position) => {
      wordArr[position] = letter;
    });
  });
  return wordArr.join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (payment === 25) {
      ...
      if (changeReserve['50'] >= 1 && changeReserve['25'] >= 1) {
        changeReserve['50'] -= 1;
        changeReserve['25'] -= 1;
        changeReserve['100'] += 1;
      } else if (changeReserve['25'] >= 3) {
        changeReserve['25'] -= 3;
      } else {
        return false;
      }
